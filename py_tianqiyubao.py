...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def categoryContent(self,tid,pg,filter,extend):		
		...
		html=jsonRemoveHeadStr(OriginalJsonTxt=html)
		...
		for vod in ListRe:
			...
			if len(lastVideo) == 0:
				lastVideo = '_'
			guid = str(id)+"###"+title+'###'+lastVideo+'###'+img
	...
	def webReadFile(self,urlStr):
		...
		return html
	def jsonRemoveHeadStr(self,OriginalJsonTxt):
		OriginalJsonTxt=OriginalJsonTxt.strip().rstrip()
		if OriginalJsonTxt[0]=="{" or OriginalJsonTxt[-1]!=")":
			return OriginalJsonTxt
		headIndex=OriginalJsonTxt.find("{",0)
		return OriginalJsonTxt[headIndex:-1]
	config = {
		"player": {},
